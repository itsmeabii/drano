...
import { useTransactions, Transaction, TransactionForm } from '@/hooks/useTransactions'
...
export default function EditTransactionModal({ transaction, onClose, onSuccess }: Props) {
  ...
  const { updateTransaction, loading, error } = useTransactions()
  ...
  const walletOptions = wallets.map((w) => ({ value: w.id, label: `${w.icon} ${w.name}` }))
  const toWalletOptions = toWallets.map((w) => ({ value: w.id, label: `${w.icon} ${w.name}` }))

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (form.type === 'transfer' && !form.to_wallet_id) return
    const success = await updateTransaction(transaction.id, form)
    if (success) {
      onSuccess?.()
      onClose()
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="border-blush relative z-10 max-h-[90vh] w-full max-w-md overflow-y-auto rounded-3xl border bg-white p-6">
        <div className="mb-6 flex items-center justify-between">
          <p className="font-display text-plum text-2xl font-semibold">Edit Transaction ✦</p>
          <button
            onClick={onClose}
              ...
              <div>
                ...
                {form.transfer_fee > 0 && (
                  <p className="text-lilac mt-1 text-xs">
                    ✦ Total deducted: {formatCurrency(form.amount + form.transfer_fee)}
                  </p>
                )}
              </div>
            </>
